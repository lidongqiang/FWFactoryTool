...
unsigned int rr;//冗余码长度		// BCH code parameters
...
#define CRC16_CCITT         0x1021  //CRC多项表达式.
void CRCBuildTable16(USHORT aPoly , USHORT *crcTable)
{
    USHORT i, j;
    USHORT nData;
    USHORT nAccum;
...
USHORT CRC_16(BYTE * aData, ULONG aSize)
{
    ULONG i;
    USHORT nAccum = 0;
    USHORT crcTable[256];
...
void P_RC4(BYTE * buf, USHORT len)
{
	...
	USHORT i,j,t,x;
	BYTE key[16]={124,78,3,4,85,5,9,7,45,44,123,56,23,13,23,17};
...
void bch_encode(BYTE *encode_in, BYTE *encode_out)
{
	UINT i,j;
	BOOL feed_back;
	UINT bch1=0;
...
USHORT CRC_Calculate(USHORT crc, UCHAR ch)
{
	UINT i;
	for(i=0x80; i!=0; i>>=1)
	{
...
USHORT CRC_CCITT(UCHAR *p, UINT CalculateNumber)
{
    USHORT crc = 0xffff;
...
void gen_poly()
{	
	unsigned int gen_roots[nn + 1], gen_roots_true[nn + 1] ; 	// Roots of generator polynomial
