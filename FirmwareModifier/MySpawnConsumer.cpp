...
CString CMySpawnConsumer::mkBootImgCmd(CString strExe,CString strKernel,CString strRamdisk,CString strOutputFile)
{
	CString strCmd;
	strCmd = _T("\"") + strExe + _T("\"") + 
...
CString CMySpawnConsumer::CovertPressionBmpCmd(CString strExe,CString strSrcPath,CString strOutputDir)
{
	CString strCmd;
	strCmd = _T("\"") + strExe + _T("\"") + _T(" -compress rle -colors 256") +
		_T(" \"") + strSrcPath + _T("\"") +
		_T(" \"") + strOutputDir + _T("\"");
	TRACE(_T("CovertPressionBmpCmd:%s\r\n"),strCmd);
	return strCmd;
}

CString CMySpawnConsumer::CovertDtstoDtb(CString strExe)
{
	CString strCmd;
	strCmd = _T("\"") + strExe + _T("\"");
	
	TRACE(_T("CovertDtstoDtb:%s\r\n"),strCmd);
	return strCmd;
}

CString CMySpawnConsumer::CovertRunBat(CString strExe)
{
	CString strCmd;
	strCmd = _T("\"") + strExe + _T("\"");
	
	TRACE(_T("CovertRunBat:%s\r\n"),strCmd);
	return strCmd;
}
