...
{    //    密钥扩展
	int i;
    const unsigned char rcon[N_ROUND][4] = { 
        {0x00, 0x00, 0x00, 0x00},
        {0x01, 0x00, 0x00, 0x00},
        {0x02, 0x00, 0x00, 0x00},
        {0x04, 0x00, 0x00, 0x00},
        ...
        for(int roundIndex = 1; roundIndex < N_ROUND; ++roundIndex)
        {
            ...
            std::swap<unsigned char>(rotWord[0], rotWord[1]);
            std::swap<unsigned char>(rotWord[1], rotWord[2]);
            std::swap<unsigned char>(rotWord[2], rotWord[3]);        
            for(int i = 0; i < 4; ++i)
            {
...
{    //    行变换
    ...
    // Do nothing.
    //state第二行循环左移一个字节
    std::swap<unsigned char>(state[4], state[5]);
    std::swap<unsigned char>(state[5], state[6]);
    std::swap<unsigned char>(state[6], state[7]);        
    //state第三行循环左移两个字节
    std::swap<unsigned char>(state[8], state[10]);
    std::swap<unsigned char>(state[9], state[11]);
    //state第三行循环左移三个字节
    std::swap<unsigned char>(state[14], state[15]);
    std::swap<unsigned char>(state[13], state[14]);
    std::swap<unsigned char>(state[12], state[13]);    
}
...
{    //    列混淆
    unsigned char matrix[4][4] = {
        ...
        {0x01, 0x01, 0x02, 0x03},
        {0x03, 0x01, 0x01, 0x02}};
        const unsigned char* temp = GFMultplyBytesMatrix((unsigned char*)matrix, state);
        for(int i = 0; i < 16; ++i)
        {
            state[i] = temp[i];
        }
        delete[] (unsigned char *)temp;
}
...
void AES::InitialState(const unsigned char* text)
...
unsigned char AES::GFMultplyByte(const unsigned char& left, const unsigned char& right)
{    //有限域GF(2^8)上的乘法
...
const unsigned char* AES::GFMultplyBytesMatrix(const unsigned char* left, const unsigned char* right)
...
const unsigned char* AES::cipher(const unsigned char* text, const unsigned char* key, const int& keySize)
{    //    用key给text加密
	int i;
    for( i = 0; i < 16; ++i)
    {
...
const unsigned char* AES::Invcipher(const unsigned char* text, const unsigned char* key, const int& keySize)
{    //    用key给text解密
	int i;
    for( i = 0; i < 16; ++i)
    {
